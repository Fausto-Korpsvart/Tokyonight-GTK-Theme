...
for theme in '' '-Blue' '-Green' '-Grey' '-Orange' '-Pink' '-Purple' '-Red' '-Teal' '-Yellow'; do
    for type in '' '-Moon' '-Storm'; do
        ...
        if [[ "$type" != '' ]]; then
            ...
            sed -i "s/#e9e9ed/${background_light}/g" "assets${theme}${type}.svg"
            sed -i "s/#1a1b26/${background_dark}/g" "assets${theme}${type}.svg"
            sed -i "s/#15161e/${base_dark}/g" "assets${theme}${type}.svg"
            sed -i "s/#323449/${surface_dark}/g" "assets${theme}${type}.svg"
        elif [[ "$theme" != '' ]]; then
